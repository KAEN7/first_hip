...
const Descendant = ({ dehydratedState: descendantList }: DescendantProps) => {
	const [activeData, setActiveData] = useState<DescendantType>(
		descendantList[0]
	);
	const keywords = descendantList
		.map((descendant) => descendant.descendant_name)
		.join(",");
	...
	return (
		<StyledDescendant gap={3} align="flex-start">
			<Head>
				...
				<meta
					property="og:url"
					content={`${process.env.NEXT_PUBLIC_DOMAIN}/descendant`}
				/>
				<meta property="og:title" content="퍼덩이 - 계승자" />
				<meta property="og:image" content="/images/logo.png" />
				<meta property="og:image:width" content="800" />
				<meta property="og:image:height" content="420" />
				<meta
					property="og:description"
					content="퍼스트 디센던트를 즐기기 위한 다양한 정보와 공략을 알아보세요, 각 계승자별 다양한 파밍 및 스킬 등 정보를 공유합니다."
				/>
				...
				<link
					rel="canonical"
					href={`${process.env.NEXT_PUBLIC_DOMAIN}/descendant`}
				/>
			</Head>
				...
				{descendantList.map((descendant: DescendantType) => {
					return (
						<Card
							...
							data={descendant}
							onClick={() => setActiveData(descendant)}
						/>
					);
				})}
			</StyledAside>
			...
			<StyledMain>
				...
				<Farming
					maxWidth={maxWidth}
					data={DESCENDANT_FARMING[activeData.descendant_name]}
				/>
...
export const getStaticProps = async () => {
	...
	await queryClient.prefetchQuery(["descendant"], () =>
		axios
			...
			.get(`${process.env.NEXT_PUBLIC_DOMAIN}/api/openApi/descendant`)
